...
### 核心功能
- 🔍 **智能城市搜索**：本地数据库 + Nominatim API 全球覆盖
- 🗺️ **多城市轨迹**：支持 A→B→C 连续路线规划
- 🚗 **多种交通方式**：驾车、火车、飞机、步行四种模式
- 💾 **轨迹保存**：保存到 SQLite 数据库，支持历史记录
- 📊 **国家城市统计**：自动统计访问的国家和城市数量（横排显示）
- 📏 **距离计算**：显示总距离和各段距离，点击轨迹线查看里程
...
2. **选择交通方式**：添加第二个城市时，选择到达该城市的交通方式（驾车/火车/飞机/步行）
...
- **修改城市名称**：在编辑模式下直接修改城市名称
- **修改交通方式**：在编辑模式下为每个城市选择不同的交通方式
- **添加城市**：搜索添加或在地图上点击添加
- **删除/移动城市**：使用上下箭头调整顺序，或删除不需要的城市

### 快捷操作

- **地图点击添加**：点击"开启地图点击添加"按钮，然后在地图上点击添加城市
- **拖动统计栏**：按住统计栏左侧拖动手柄移动位置
- **查看统计信息**：点击统计栏展开查看按国家分组的城市列表
